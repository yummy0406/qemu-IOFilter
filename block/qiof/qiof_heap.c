...
{
	size_t align = 0;
	size_t addr = (unsigned long)(*pBuf);
...
memory_chunk *create_list(memory_chunk *pool, size_t count) // 双向链表 从链表头部插入
{
	if (!pool)
	{
		return NULL;
...
/* 内存映射表中的索引转化为内存起始地址  */
/************************************************************************/
void *index_to_addr(PMEMORYPOOL mem_pool, size_t index);
void *index_to_addr(PMEMORYPOOL mem_pool, size_t index)
{
...
/* 内存起始地址转化为内存映射表中的索引   */
/************************************************************************/
size_t addr_to_index(PMEMORYPOOL mem_pool, void *addr);
size_t addr_to_index(PMEMORYPOOL mem_pool, void *addr)
{
...
/* 重新初始化内存池大小*/
/************************************************************************/
void ReSizeMemoryPool(void *pBuf, size_t sBufSize)
{
	pBuf = (void *)realloc(pBuf, sBufSize);
	if (pBuf == NULL)
	{
		return;
	}
...
void *GetMemory(size_t sMemorySize, PMEMORYPOOL pMem)
{
	...
	if (tmp->pfree_mem_addr->count * MINUNITSIZE == sMemorySize)
	{
		...
		size_t current_index = (tmp->pfree_mem_addr - pMem->pmem_map); // addr in map
		...
		return index_to_addr(pMem, current_index);
	}
	else
	{
...
{
	size_t current_index = addr_to_index(pMem, ptrMemoryBlock);
	...
	// 最后一个
	else if (current_index == pMem->mem_block_count - 1) // 可能错误01：判断是否为最后1个的if条件有错误!
	{
		if (current_block->count < pMem->mem_block_count)
		{
